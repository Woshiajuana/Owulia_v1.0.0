...
;(function (window,doc,_a,undefined) {
    ...
    var Carousel = function (ele,opt) {
        ...
        this.ele = ele || document.getElementById('carousel');//需要滚动的大盒子，默认是ID为carousel的元素

        this.sleep = opt.sleep || 3000;//每隔多少秒滚动一次，默认是3000ms

        this.times = opt.times || 300;//滚动一次需要的时间，默认是300ms

        this.nextBtn = opt.nextBtn;//向下按钮

        this.prevBtn = opt.prevBtn;//向上按钮

        this.triggerBox = opt.triggerBox;//触发的按钮

        this.triCss = opt.triCss;//触发的样式

        this.callback = opt.callback;//回调函数

        this.boxHover = opt.boxHover || false;

        this.triHover = opt.triHover || false;
    ...
    Carousel.prototype.init = function () {
        ...
        if(this.sleep) alwaysNextFun(this);//触发连续滚动
    ...
    //操作元素的方法
        reviseEleFun = function (that) {
            ...
            //复制i标签到trigger里面
            if(that.triggerBox && !that.triggerBox.childNodes.length){
